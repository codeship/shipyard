...
RSpec.describe Shipyard::IconHelper, type: :helper do
  ...
  xit 'should return an svg logo icon with the svg use tag.' do
    expect(icon(:logo)).to match(/<svg class="icon icon-logo icon-outline"(.+?)><use(.+?)>/)
  end

  xit 'should return an svg logo icon without the svg use tag.' do
  ...
  xit 'should return an svg logo icon with a custom class name' do
